// histogram.js
export function initHistograminteract(d3) {
    // -----------------------histogram------------------------
    ...
    const histMargin = { top: 5, right: 15, bottom: 45, left: 45 };

    const svgHistogram = d3.select("#histograminteract").append("svg")
  ...
  //function to update the informatin in the histogram based on the country clicked
  export function updateHistogram(volcanoData, selectedCountry = null) {
    // Clearing the existing histogram before redrawing
    d3.select("#histograminteract svg").remove();

    //filtering the data to change the histogram to show data about a selected coutry
    const filteredData = selectedCountry ? volcanoData.filter(d => d.country === selectedCountry) : volcanoData;

    // Converting elevation to numeric
    filteredData.forEach(function(d) {
        d.elevation = +d.elevation;
    });

    //making the ranges for binning
    const elevationRanges = d3.range(-2500, 7000, 1000);

    //making the thresholded binned data
    let binnedData = d3.bin()
        .value(d => d.elevation)
        .thresholds(elevationRanges)
        (filteredData);
